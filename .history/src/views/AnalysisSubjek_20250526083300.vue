...
const toggleSelect = (code) => {
  if (selectedSubjects.value.includes(code)) {
    selectedSubjects.value = selectedSubjects.value.filter((c) => c !== code);
  } else {
  ...
  <div class="bg-gray-100 min-h-screen">
    ...
    <main>
      ...
      >
        ...
        <h2 class="text-2xl font-bold drop-shadow-md">Analisis Subjek</h2>
        ...
        <table class="w-full border border-black text-sm text-center bg-[#d0e7f7]">
          <thead class="bg-[#b8d4ea]">
            <tr>
              ...
              <th class="border border-black px-2 py-1">Kursus</th>
              <th class="border border-black px-2 py-1">Bil. Pelajar</th>
              <th class="border border-black px-2 py-1">Fakulti (Pelajar)</th>
              <th class="border border-black px-2 py-1">Pilih Item</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(subject, index) in subjectList" :key="index">
              ...
              <td class="border border-black px-2 py-1">{{ subject.course }}</td>
              <td class="border border-black px-2 py-1">{{ subject.students }}</td>
              <td class="border border-black px-2 py-1">{{ subject.faculty }}</td>
